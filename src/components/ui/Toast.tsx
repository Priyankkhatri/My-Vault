...
const borderColors: Record<Toast['type'], string> = {
 success: 'border-vault-success',
 error: 'border-vault-danger',
 warning: 'border-vault-warning',
 info: 'border-vault-info',
};
...
export function ToastContainer() {
 ...
 className={`flex items-center gap-3 px-4 py-3 border ${borderColors[toast.type]}`}
 >
 {icons[toast.type]}
 <span className="text-sm text-vault-text flex-1">{toast.message}</span>
 <button
 onClick={() => removeToast(toast.id)}
 className="text-vault-text-muted cursor-pointer"
 >
 <X size={14} />
 </button>
 </motion.div>
